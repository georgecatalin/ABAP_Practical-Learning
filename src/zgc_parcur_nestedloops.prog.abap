...
*&---------------------------------------------------------------------*
*& Author: George Calin
*& Description: This report demonstrates how to create to use parralel cursor with nested loops for better performance
*&---------------------------------------------------------------------*
REPORT zgc_parcur_nestedloops.
...
* Fill in the lt_complete internal tables that contains all the data in the application layer which will be sent to the screen in the presentation layer
  ...
  LOOP AT lt_header INTO ls_header.
    ...
    READ TABLE lt_item INTO ls_item WITH KEY ordernumber = ls_header-ordernumber.
    IF sy-subrc = 0.
      lv_index = sy-tabix.
    ENDIF.
    ...
    IF ls_item-ordernumber <> ls_header-ordernumber.
        EXIT.
    ENDIF.
